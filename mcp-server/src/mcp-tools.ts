// Register a tool specifically for testing notification
  import {
...
import { type CallToolResult, isInitializeRequest, JSONRPCMessageSchema } from '@modelcontextprotocol/sdk/types.js';
...
export default async function initMcpTools(server: FastMCP, waLogger: P.Logger, mcpLogger: P.Logger, sock: WhatsAppSocket | null) {
    
  server.addTool({
    name: 'random_tool',
    description: 'Starts sending wa notifications upon call',
    parameters: z.object({
      name: z.string().describe('Name to send notifications for')
    }),
    execute: async (args) => {
      return {
        content: [
  ...
  server.addTool({
    ...
    description: "Starts server to client whatsapp notifications for messages via logging",
    parameters: z.object({}),
      ...
      sock?.ev.on('messages.upsert' , async (messageUpdate) => {
        if (messageUpdate.type === 'notify') {
          for (const msg of messageUpdate.messages) {
            try {
              console.error(`New message : ${msg ? JSON.stringify(msg) : 'No content'}`);
              if (!msg.key.remoteJid || !msg.message || !msg.message.conversation) {
                continue;
              }
              log.info(msg.message.conversation, msg.key.remoteJid);
            } catch (error) {
              console.error('Error in mcp server get-log', error);
            }
          }
        }else{
          for (const msg of messageUpdate.messages) {
           console.error(`New message type 2 : ${msg ? JSON.stringify(msg) : 'No content'}`);
          }
        }
      })
    },
  ...
  server.addTool({
    ...
    execute: async (args) => {
      mcpLogger.info(`[MCP Tool] Executing send_message to ${args.recipient}`);
      if (!sock) {
        mcpLogger.error(
      ...
      try {
        normalizedRecipient = jidNormalizedUser(args.recipient);
        if (!normalizedRecipient.includes("@")) {
          throw new Error('JID must contain "@" symbol');
        }
      } catch (normError: any) {
        mcpLogger.error(
          `[MCP Tool Error] Invalid recipient JID format: ${args.recipient}. Error: ${normError.message}`,
        );
        return {
          isError: true,
          content: [
            {
              type: "text",
              text: `Invalid recipient format: "${args.recipient}". Please provide a valid JID (e.g., number@s.whatsapp.net or group@g.us).`,
            },
          ],
        };
      }
      ...
      try {
        ...
        mcpLogger.error(
          `[MCP Tool Error] send_message failed for ${args.recipient}: ${error.message}`,
        );
        return {
          isError: true,
          content: [
  ...
  server.addTool({
    ...
    description: "Searches wa contacts given contact name or phone number or JID",
    parameters: z.object(
      {
        query: z
          .string()
    ...
    execute: async (args) => {
      mcpLogger.info(
        `[MCP Tool] Executing search_contacts with query: "${args.query}"`,
      );
      try {
        const contacts = searchDbForContacts(args.query, 20);
  ...
  server.resource("db_schema", "schema://whatsapp/main", async (uri: URL) => {
